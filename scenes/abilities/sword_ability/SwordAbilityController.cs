...
public partial class SwordAbilityController : Node
{
    ...
    [Export]
    public PackedScene SwordAbilityScene { get; private set; }
    ...
    private void HandleTimerTimeout()
    {
        ...
        mobs = new Godot.Collections.Array<Node>(
            mobs.OrderBy(mob =>
                ((Node2D)mob).GlobalPosition.DistanceSquaredTo(player.GlobalPosition)
            )
        );
        ...
        if (
            closestMob.GlobalPosition.DistanceSquaredTo(player.GlobalPosition)
            > Math.Pow(MaxRange, 2)
        )
            return;

        // Instantiate sword in the main scene...
        SwordAbility swordInstance = (SwordAbility)SwordAbilityScene.Instantiate();
        ...
        swordInstance.GlobalPosition =
            closestMob.GlobalPosition
            + Vector2.Right.Rotated((float)GD.RandRange(0.0f, Math.Tau)) * 4;
